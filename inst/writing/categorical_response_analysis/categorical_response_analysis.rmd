...
mapping or by converting to \{0,1\} dummy variables. This memo argues why broad application of this practice is
problematic statistically and offers alternative solutions to the issue.

For those just looking to see the suggested methods for dealing with nominal categorical variables, please skip ahead
to section titled "How to avoid dummy response variables".
...
")
variable_tree_path <- tempfile()
variable_tree %>%
  export_svg() %>%
  charToRaw() %>%
  rsvg_png(variable_tree_path)
knitr::include_graphics(variable_tree_path)
```

A variable is quantitative if it is inherently ordered and have an internally consistent, scalable definition of
difference. For example, consider a variable representing age. 

For our purposes, we can characterize data as being either categorical and quantitative although both of these terms
have potential for misunderstanding. A variable is quantitative if it is inherently ordered and have an internally
consistent, scalable definition of difference. A variable is categorical if all of its values are disjoint, and it does
not necessarily have a scalable definition of difference; however, a categorical variable can be 
ordered [@SINHARAY20101].


It is important to consider that many variables can be interpreted to be of either category. For example, consider
age in the form of number of years. This variable can be interpreted as quantitative and is often encoded as just a
number. However, it can be encoded as a categorical without loss of information, as in the case when we make a variable
designating if a child is 3 or 4 years old. Likert scales are a classic example of an ordered categorical variable;
however, these are often re-encoded into 1-5 scales for the purposes of measure creation or aggregation despite the 
lack of rigor in ensuring that the difference between "Strongly Disagree" and "Disagree" is exactly the same as 
"Disagree" and "Indifferent". Unordered categorical variables like race appear safer to this mess of ambiguity but
even these can be thought of as quantitative through the use of dummy variables.

Dummy variables serve as a multivariate encoding of a univariate categorical variable where each dummy variable
corresponds to a specific possible value of the categorical variable. Dummy variables are typically coded with a value 
space of 0, 1 and `NA` or the language equivalent missing for their convenient mathematical properties. It is
typical for a variable with $n$ possible values to create $n$ new variables; however, creating $n-1$ variables contains
the same amount of information for reasons we'll get into later.
...
as separate classes of vector in R. Quantitative variables should be represented by the numeric class (which isn't
technically a class but that distinction doesn't matter here), nominal categorical variables  should be represented by
the factor class and ordered categorical variables should be possess both the "ordered" and "factor" classes. 

To clear up a potentially very impactful misunderstanding, we need to discuss ordered factors. As stated previously,
these are used to encode ordinal categoricals. Because of the analytical implications when using ordered factors, I do
\textbf{NOT} recommend re-classing variables as ordered factors unless you know that the corresponding implications.
Skip ahead if you don't want to learn about how these classes work internally.

### A slightly deeper dive in factors and their implementation in $R$

R has 25 different types of vectors at its lowest, most relevant here are `integer`, `double`,
...
is `ordered`, the integers do **NOT** store or intend any meaning as a number. Thy are only stored as
`integer` because it is space efficient to do so. This implementation, incidentally, is what makes the factor
much more space efficient in the storage of large amount of character data provided there is sufficient replication of
values. Adding `ordered` as a class to a factor, usually by setting `ordered = TRUE` when defining
a factor just adds `ordered` as a class but doesn't do anything else to the underlying implementation.
...
the model normally but an ordered `factor` is actually treated with a polynomial model. Neither treatment
is inherently wrong, it is just important that you select the appropriate method for your data and purpose.

The crux of this confusions arises because the order of strings in the `levels` attribute in an unordered
...
when calculating contrasts as the "baseline". This doesn't at all change the computation of the model but it does affect
...
`levels` attribute is sufficient.

# Why not to use dummy variables

The proper treatment of ordinal categorical variables is outside of the scope of this memo. Ordinal categorical
variables actually cannot naively reduced into dummy variables without the loss of the ordering so it is a bit of a moot
point regardless.
...
in your formula (although I do think it's important to note here that even this practice is vulnerable to "p-hacking"
abuse). It seems logical then to just do the same with a response variable; just run the model 
...
collinear with a set of non-statistically significant variables. This issue of interpretability suggests then that
we would want a single, shared statistical test when evaluation a categorical variable such that we have a single
measure of significance.

Another important consideration pertains the implications of the definition of a p-value. To define in non-rigorous 
terms, a p-value of 0.05 states that given the null hypothesis is true, there is a \SI{5}{\percent} chance that the
actually found result could have occurred. Implicit with this is the fact that there is an underlying assumption that
these observed differences can occur even if the null hypothesis is true in reality. This can be effectively abused,
intentionally or otherwise, to generate significant p-values by running ever more hypothesis tests as 5%
will satisfy our arbitrary condition by random chance [@Stefan2023]. Running a test on multiple dummy variables provides
a seemingly innocent way to accomplish. Granted the actual mathematical implications are more complex than what I'm
saying here because of the inherent collinearity present but below I present my case with simulated data.
...
A quick glance at the code above suggests that there is absolutely no relationship between the intervention group
and any of the nominal categorical generated. Running a simple linear model and looking at the results suggests 
otherwise.
...
Lets take a look at how the p-value stars are distributed. Remember again that the parametric truth of the dataset is
that no differences are statistically significant.

```{r, echo = FALSE}
count(test_comparisons, `P-value Stars` = Stars_Intervention_Control)
```

This truth is shown in 75 of our 82 tests; however, there are 7 tests that display some significance with a p-value of 
0.1 or lower, or 8.5% of the results. Taking a look at which tests display significance we see results
that clearly look significant when viewed independently with some of our "Letters" and "State" dummy variable tests.
However, it doesn't really make sense to say that this treatment affects whether the "Letters" variable is "f" or "n" but
not any other variable. Using these dummy response variables doesn't provide a rigorous mechanism for us to seen or
communicate the fact that these internally, statistically significant results should be see considering the absence
of significance in the related dummy variables. Reporting on these dummies allow us to artificially generate
statistically significant results, especially if these are reported without robust mention of the other, non-significant
results.
...
the selection of category boundaries itself is a method that, intentionally or not, can be used to generate false
significance [@Stefan2023]. However, if we still want to or must use a categorical variable as our response, there 
exist methods. Technically, binary dependent variables "should" be run with a logistic regression model although
...
to determine the relationship between two categorical variables, e.g. a race and your research group. It is not useful
...
particularly used at MDRC for early investigations of data regarding baseline equivalence and non-response bias, both
...
Noting again that a logistic model is the technically proper treatment for a categorical outcome with 2 levels, a
multinomial logistic model practically generalizes the method for arbitrarily many levels. The 
associated
\href{https://mdrc365.sharepoint.com/:b:/r/sites/QuantMethods/Shared%20Documents/Guidance/QMG%20Categorical%20Outcomes%20Memo%20(December%202013).pdf?csf=1&web=1&e=wqRkvO}{QMG memo}
also makes note of a Seemingly Unrelated Model to analyze categorical outcomes but that is also outside of the scope of
this memo.
...
\begin{itemize}
  \item Given every other covariate in the model, being in the treatment group increases the odds of being in the "croc"
  category vs the baseline "cat" category by 1.21.
  \item Given every other covariate in the model, being in the treatment group increases the odds of being in the "dog"
  category vs the baseline "cat" category by 1.57.
  \item The $p$-value indicates the probability that, given all other coefficients are present, that the specified
...
The QMG memo actually recommends supplementing MNL with $G$-computation to calculate proper p-values and estimates on
...
If we want to calculate the treatment effect on `pre_income_raw`, a quantitative variable, we can simply specify the
model as such using the `lme4` package. I am also calling the `emmeans` for adjusted means calculations and
`broom.mixed` for its method extensions on `broom`.
...
Effect sizes are calculated individually using the dummy variables as outcomes; however, we will be ignoring the
...
mixed_effect_sizes <- paste0("favAnimals_", c("cat", "croc", "dog")) %>%
...
```{r}
mixed_effect_sizes %>%
  mutate(Categorical_Level, Effect_Size,
...
estimate. First to calculate the p-value, we create a new dataset by aggregated by our nesting variable.
...
Now we calculate two linear models, a null and effect model. Both models use our dummy `treatment` variable as a 
...
The actual effect sizes are calculated using a multi-level model for each dummied predictor.
...
`lme4::glmer`. One method I saw in the `lme4` github would be to convert a multinomial model into
the equivalent Poisson regression [@lee2017poisson]. `lme4::glmer` supports every GLM family listed in
`stats::glm` which does includes Poisson regression which means we can use this equivalent Poisson formulation
in `lme4::glmer`.
